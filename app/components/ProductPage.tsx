...
import { CareJourney, careJourney } from "./CareJourney";
import { PageSectionLegend } from "./PageSectionLegend";
import { TreatmentCard } from "./TreatmentCard";

export function ProductPage({product}: {product: Product}) {
  const [stage,setStage] = useState(0);
  const compounded = product.slug === "glp-1-injections" || product.slug === "glp-1-tablets";
  ...
  useEffect(()=>{
    if(compounded||window.matchMedia("(prefers-reduced-motion: reduce)").matches)return;
    const timer=window.setInterval(()=>setStage(current=>(current+1)%stageViews.length),5000);
  ...
  const productSections = [
    ...
    {id:"care",label:"Care"},
    {id:"compare",label:"Options"},
    {id:"important-safety",label:"Safety"},
  ];
  return <main className={`detail-page product-experience product-${product.slug}`}>
    <SiteHeader />
    <PageSectionLegend items={productSections} label={`${product.shortName} page sections`} />
    <section className="product-hero-detail" id="overview">
      <div className="product-hero-copy"><p className="detail-kicker">{product.status}</p><h1>{product.name}</h1><p className="product-ingredient">with {product.ingredient}</p><ul>{product.highlights.map(item => <li key={item}>{item}</li>)}</ul><Link className="detail-primary" href="/eligibility/weight-loss">Check eligibility</Link><Link className="safety-link" href="#important-safety">Important safety information</Link></div>
      <div className={`product-hero-product stage-${stageViews[stage].kind}`}><div className="product-aura"></div><div className="product-stage-number">0{stage+1}</div><img src={stageViews[stage].image} alt={stageViews[stage].alt} /><span>{stage===0?product.cadence:stageViews[stage].label}</span>{!compounded&&<div className="product-view-switcher" aria-label="Change product view">{stageViews.map((view,index)=><button key={view.label} className={stage===index?"active":""} onClick={()=>setStage(index)} aria-label={`Show ${view.label.toLowerCase()} view`} aria-pressed={stage===index}><i></i><small>{view.label}</small></button>)}</div>}</div>
    </section>
    ...
    {product.slug!=="zepbound-injection"&&<section className={`journey-page ${compounded?"compounded-program-journey":""}`} id="care"><div className="journey-page-heading"><p className="detail-kicker">The nine-stage journey</p><h2>From program selection to reassessment.</h2><p>Rejuvonix previews every stage while independent providers and pharmacies remain responsible for clinical care and fulfillment.</p></div>{compounded?<div className="journey-layout"><div className="journey-photo"><img src="/rejuvonix-clinician.png" alt="A clinician guiding a patient through a compounded care journey"/><span>Private, provider-led care</span></div><div className="journey-steps">{careJourney.map(([number,title,copy])=><article key={number}><span>{number}</span><div><h3>{title}</h3><p>{copy}</p></div></article>)}</div></div>:<CareJourney/>}</section>}
    ...
    <section className="compare-detail unified-treatment-section" id="compare"><div className="compare-heading"><p className="detail-kicker">Compare options</p><h2>Which treatment may be right for you?</h2><p>A licensed provider reviews your intake and determines what is medically appropriate.</p></div><div className="treatment-grid branded-grid related-treatment-grid">{related.map(item=><TreatmentCard product={item} index={products.indexOf(item)} key={item.slug}/>)}</div><Link className="detail-secondary" href="/treatments">View all treatments</Link></section>
    <section className="future-banner"><img src={product.heroImage} alt={product.shortName} /><div><p className="detail-kicker">Care for the long term</p><h2>Build a plan you can understand and follow.</h2><Link className="detail-primary" href="/eligibility/weight-loss">Check eligibility</Link></div></section>
    ...
    <section className={`detail-closing ${compounded?"compounded-closing":""}`}>{compounded&&<div className="compounded-closing-graphic" aria-hidden="true"><span><img src="/compounded-semaglutide-transparent.png" alt=""/></span><span><img src="/compounded-tirzepatide-transparent.png" alt=""/></span></div>}<p className="detail-kicker">Get started</p><h2>{compounded?"Find out which compounded option may fit your care plan.":"Find out which option may fit your health needs."}</h2><Link className="detail-primary" href="/eligibility/weight-loss">Start the assessment</Link><small>A prescription is not guaranteed.</small></section>
    <SiteFooter />
  </main>;
}
