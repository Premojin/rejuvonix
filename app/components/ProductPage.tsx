...
export function ProductPage({product}: {product: Product}) {
  ...
  const intakeHref = MAIN_GLP_INTAKE_URL;
  ...
  return <main className={`detail-page product-experience product-${product.slug}`}>
    ...
    <section className="product-hero-detail" id="overview">
      <div className="product-hero-copy"><p className="detail-kicker">{product.status}</p><h1>{product.name}</h1><p className="product-ingredient">with {product.ingredient}</p><ul>{product.highlights.map(item => <li key={item}>{item}</li>)}</ul><Link className="detail-primary" href={intakeHref}>Check eligibility</Link><Link className="safety-link" href="#important-safety">Important safety information</Link></div>
    ...
    <section className="future-banner"><img src={product.heroImage} alt={product.shortName} /><div><p className="detail-kicker">Care for the long term</p><h2>Build a plan you can understand and follow.</h2><Link className="detail-primary" href={intakeHref}>Check eligibility</Link></div></section>
    ...
    <section className={`detail-closing ${compounded?"compounded-closing":""}`}>{compounded&&<div className="compounded-closing-graphic" aria-hidden="true"><span><img src="/v6/compounded-medication-generic.png" alt=""/></span><span><img src="/v6/compounded-medication-generic.png" alt=""/></span></div>}<p className="detail-kicker">Get started</p><h2>{compounded?"Find out which compounded option may fit your care plan.":"Find out which option may fit your health needs."}</h2><Link className="detail-primary" href={intakeHref}>Start the assessment</Link><small>A prescription is not guaranteed.</small></section>
    <SiteFooter />
  </main>;
}
