...
export default function AuthNav({ mobile = false }: { mobile?: boolean }) {
  ...
  if (state === "authenticated") return <span className={mobile ? "auth-nav-mobile" : "auth-nav"}><span className="auth-signed-in" aria-label="Signed in">Signed in</span><Link href="/account">Account</Link><button type="button" onClick={handleSignOut}>Sign out</button></span>;
