...
// COA files are rendered only after an exact item/certificate match is verified and imported locally.
type BaseProtocolItem = Omit<ProtocolItem, "documentation">;
...
export const protocols: ProtocolItem[] = baseProtocols.map((protocol) => ({
  ...
  documentation: documentationManifest[protocol.slug] ?? {
    ...
    label: "Documentation pending verification",
  },
}));
