...
export default function EligibilityFlow({program,entryLabel}: {program:ProgramSlug;entryLabel?:string}){
  ...
  return <main className="eligibility-page"><a className="skip-link" href="#assessment-content">Skip to assessment</a><header className="eligibility-header"><Link className="eligibility-brand" href="/" aria-label="Rejuvonix home"><img src="/rejuvonix-logo-mark.png" alt=""/>REJUVONIX<span>.</span></Link><Link className="eligibility-switch" href="/eligibility">Change program</Link></header><div className="eligibility-progress" role="progressbar" aria-label="Assessment progress" aria-valuemin={0} aria-valuemax={totalSteps} aria-valuenow={step+1} aria-valuetext={`Step ${step+1} of ${totalSteps}`}><span style={{width:`${progress}%`}}/></div><section className="eligibility-shell"><aside className="eligibility-story"><p>{entryLabel ?? config.kicker}</p><h1>{config.headline}</h1><p>{config.description}</p><div className="eligibility-trust"><span aria-hidden="true">✓</span><div><strong>Provider review comes first</strong><small>These questions do not determine medical eligibility.</small></div></div></aside><div id="assessment-content" className="eligibility-card" ref={cardRef} tabIndex={-1}>{step>0&&<button type="button" className="eligibility-back" onClick={()=>setStep(current=>Math.max(0,current-1))}>← Back</button>}
    ...
    {isHandoff&&<div className="eligibility-question eligibility-handoff"><span className="handoff-mark" aria-hidden="true">✓</span><p className="eligibility-kicker">Preview complete</p><h2>Continue to secure patient access.</h2><p>Your answers remain a non-submitting preview. Create or access your secure patient account when you are ready for the next approved step.</p><p><strong>A licensed provider, not this website, determines whether treatment is appropriate and establishes any medication, dose or treatment schedule.</strong></p><div className="handoff-actions"><Link className="eligibility-continue" href="/sign-up">Create a secure account <span aria-hidden="true">→</span></Link><Link className="eligibility-signin" href="/patients/login">Sign in to your account</Link></div><Link className="eligibility-secondary" href="/eligibility">Choose another program</Link><small>Preview responses are not submitted or saved. A prescription is not guaranteed.</small></div>}</div></section></main>;
}
